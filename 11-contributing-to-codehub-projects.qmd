---
output-file: contributing-to-ihealth-projects.html
---

# Contributing to Oxford iHealth projects {#sec-contributing-ihealth}
...
The Oxford iHealth actively runs projects focused on either research software development (based in R) or R-based scientific research workflows. Our list of previous and current projects can be found [here](https://oxford-ihtm.io/projects/).

## Research software development {#sec-research-software}

Oxford iHealth develops research software using the R package system. Our collection of research software tools are found [here](https://oxfordihtm.r-universe.dev) and the underlying code is available from our [organisational GitHub](https://github.com/OxfordIHTM).

If you would like to contribute to Oxford iHealth's software development, following is a list of steps on how to.
...
Visit the packages page of the Oxford iHealth website - [https://oxford-ihtm.io/packages/](https://oxford-ihtm.io/packages/) to see our current line-up of Oxford iHealth software projects.
...
This communication is important as this will trigger the developer/maintainer to confirm that you are an eligible member of the Oxford iHealth (the pre-requisites above) and then will add you as a collaborator on the project. This step is important because your status in the project as a collaborator will determine your next step on contributing. We prefer that Oxford iHealth members are internal collaborators as this simplifies their participation (see next section).
...
These forking steps are described in all the CONTRIBUTING notes/guidance in each project as we open contributions from anyone (not just Oxford iHealth members) but not all are eligible to be internal collaborators.

We would like to avoid this for eligible members of Oxford iHealth so will endeavour to keep up with collaborator requests. So, if you are impatient and would really like to contribute already, consider forking but be warned that this has a lot more complicated steps than simple cloning. Either follow-up your request to be made a collaborator.
